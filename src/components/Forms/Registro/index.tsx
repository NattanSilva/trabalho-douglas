...
export default function Registro(props: Props) {
  const form = useForm<Pokemon>({
    resolver: zodResolver(pokemonSchema),
    defaultValues: props.defalutValues,
  })
  const router = useRouter()

  const handleResgitPoekemon = async (data: Pokemon) => {
    const dados = {
      name: data.name,
  ...
  return (
    <Card className='w-[85%]'>
      ...
      <CardContent>
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleResgitPoekemon)}
